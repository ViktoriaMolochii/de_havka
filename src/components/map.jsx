...
const Map = ({ sections, userCoords }) => {

    useEffect(() => {
        loader.load().then(() => {
            map = new google.maps.Map(document.getElementById("map"), {
                center: isEmpty(userCoords) ? { lat:49.84926512507221, lng: 24.021481756414516} : userCoords ,
                zoom: 20.5,
            });
            ...
            arr.Marker_Fish=new google.maps.Marker({
                ...
                icon: {
                    ...
                    fillColor: "#0000ff",
                    fillOpacity: !isEmpty(userCoords) ? 1 : 0,
                    scale: 0.075,
                },
            });
            ...
            arr.Marker_Fish=new google.maps.Marker({
                ...
                map,
                title: "Риба",
                opacity: sections[0].toString() === MARKERS.FISH.lng && sections[1].toString() === MARKERS.FISH.lat ? 1 : 0,
            });
            arr.Marker_Press=new google.maps.Marker({
                position:{ lat:49.84918152938752, lng:  24.021356161987615},
                map,
                title: "Преса",
                opacity: sections[0].toString() === MARKERS.PRESS.lng.toString() && sections[1].toString() === MARKERS.PRESS.lat.toString() ? 1 : 0,
            });
            arr.Marker_Bakery=new google.maps.Marker({
                position:{ lat:49.849324212369375, lng:  24.021576103123444},
                map,
                title: "Хлібна майстерня",
                opacity: sections[0].toString() === MARKERS.BREAD.lng.toString() && sections[1].toString() === MARKERS.BREAD.lat.toString() ? 1 : 0,
            });
            arr.Marker_Meat=new google.maps.Marker({
                position:{ lat:49.849278218557416, lng: 24.021326221209076},
                map,
                title: "М'ясо",
                opacity: sections[0].toString() === MARKERS.MEAT.lng.toString() && sections[1].toString() === MARKERS.MEAT.lat.toString() ? 1 : 0,
            });
            arr.Marker_Vegetables=new google.maps.Marker({
                position:{ lat:49.84922374193378, lng:  24.02156667467375},
                map,
                title: "Фрукти та овочі",
                opacity: sections[0].toString() === MARKERS.FRUITS.lng.toString() && sections[1].toString() === MARKERS.FRUITS.lat.toString() ? 1 : 0,
            });
